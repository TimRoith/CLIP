...
def train_and_save_models():
    ...
    # Create models
    model = fully_connected(sizes, "ReLU").to(device)
    model_max = fully_connected(sizes, "ReLU").to(device)
    model_adv = fully_connected(sizes, "ReLU").to(device)

    # Create trainers
    trainer = Trainer(model, xy_loader, 100, lamda=lamda, loss=F.cross_entropy, lr=0.001, adversarial_name="Adam", num_iters=5, min_accuracy=min_accuracy, CLIP_estimation="sum", iter_warm_up=warm_up_iter, change_lamda_in=False)
    trainer_max = Trainer(model_max, xy_loader, 100, lamda=lamda, lr=0.001, loss=F.cross_entropy, adversarial_name="Adam", num_iters=5, min_accuracy=min_accuracy, CLIP_estimation="max", iter_warm_up=warm_up_iter, change_lamda_in=False)
    trainer_adv = Trainer(model_adv, xy_loader, 100, lr=0.001, loss=F.cross_entropy, min_accuracy=min_accuracy, iter_warm_up=warm_up_iter)
    ...
    for i in range(num_total_iters):
        ...
        if i % 1 == 0:
            print(f"Epoch {i}")
            print("Sum CLIP: Train accuracy:", trainer.train_acc, "Train loss:", trainer.train_loss)
            print("Max CLIP: Train accuracy:", trainer_max.train_acc, "Train loss:", trainer_max.train_loss)
            print("Adversarial Training: Train accuracy:", trainer_adv.train_acc, "Train loss:", trainer_adv.train_loss)

    # Save models
    torch.save(model.state_dict(), model_sum_clip_path)
    torch.save(model_max.state_dict(), model_max_clip_path)
    torch.save(model_adv.state_dict(), model_adv_training_path)
...
def load_models_and_test(epsilon_test=False):
    ...
    # Load saved models
    if os.path.exists(model_sum_clip_path):
        model.load_state_dict(torch.load(model_sum_clip_path))
        print("Loaded sum CLIP model.")
    else:
        print("sum CLIP model file not found.")
    
    if os.path.exists(model_max_clip_path):
        model_max.load_state_dict(torch.load(model_max_clip_path))
        print("Loaded max CLIP model.")
    else:
        print("max CLIP model file not found.")
    
    if os.path.exists(model_adv_training_path):
        model_adv.load_state_dict(torch.load(model_adv_training_path))
        print("Loaded adversarial training model.")
    else:
        print("Adversarial training model file not found.")
